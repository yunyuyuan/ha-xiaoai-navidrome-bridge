...
该页面是集成注册的原生 `panel_custom`，不是 Lovelace 仪表盘，因此不出现在 Home Assistant 的“仪表盘”管理页。管理员可进入 **设置 → 设备与服务 → XiaoAI Navidrome → 配置**，修改 **侧栏和页面名称**、选择 **页面语言**，或关闭 **显示侧栏面板**。名称会同时用于侧栏条目和页面标题。页面语言只支持英语和简体中文，默认英语，并且只能在该配置页修改；主页和 Panel 本身不显示语言控件。关闭开关只注销侧栏入口；集成运行时、语音口令、服务动作、持久队列和自动切歌保持运行。重新打开开关会在配置重载后恢复入口。
...
如果侧栏没有出现配置的页面名称，先确认 Config Entry 已成功加载并以管理员账户登录。浏览器强制刷新可清理静态资源缓存；静态资源 URL 包含集成版本，正常更新会自动换用新资源。
